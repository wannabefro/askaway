...
Ember.SimpleAuth.Session = Ember.ObjectProxy.extend({
  ...
  */
  setup: function(authenticator, content) {
    ...
    try {
      this.content.user = JSON.parse(this.content.user);
    }
    catch (e){
      this.content.user = this.content.user;
    }
    this.bindToAuthenticatorEvents();
    var data = Ember.$.extend({
      authenticator: this.authenticator.toString()
    }, this.content);
    this.store.clear();
  ...
  */
  createAuthenticator: function(className) {
    if (className){
      ...
      var authenticator = className.substring(1);
    } else {
      var authenticator = className;
    }
    var authenticatorClass = classifyString(authenticator);
